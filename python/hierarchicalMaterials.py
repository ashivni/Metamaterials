...
class bond:
    def __init__(self, n1, n2, bond_type):
...
class hexagonal_unit_cell:
    ...
    def add_neighbor(self, neighbor, neigh_type):
        ...
        if neighbor._id in self._neighbors and self._id in neighbor._neighbors:
            raise ValueError(
                "[Attempting neighboring] Unit cells %s and %s are already neighbors" % (self._id, neighbor._id))
        elif neighbor._id in self._neighbors or self._id in neighbor._neighbors:
            raise ValueError(
                "[Attempting neighboring] Unit cells %s and %s are partial neighbors" % (self._id, neighbor._id))
        else:
...
class hexagonal_grid:
    def __init__(self, nx, ny, level, periodic=False, **kwargs):
        ...
        self._master_grid = {}
        print "Hexagonal Grid: ", (nx, ny, level, self._l0, self._magnification, self._a)
        self._all_cells = set()
...
class hierarchical_grid:
    def __init__(self, nx, ny, levels, **kwargs):
        ...
        self._outline = hexagonal_grid(self._nx, self._ny, self._levels, periodic=False, l0=self._l0,
        ...
        while _l >= 0:
            ...
            for _bond_id, _bond in self._level_bonds[_l].iteritems():
                _n1, _n2 = _bond._n1, _bond._n2
                if _bond_id not in _duplicate_bonds and _n1._id > _n2._id:
                    ...
                    for neigh_id in neighs:
                        ...
                        if node_x - _i_min <= _notch_len_ind and (node_y - _j_mid) * (neigh_y - _j_mid) < 0:
                            ...
                            if node_id + '_' + neigh_id in self._level_bonds[_l]:
                                _notched_bonds[node_id + '_' + neigh_id] = self._level_bonds[_l].pop(
                                    node_id + '_' + neigh_id)
                            if neigh_id + '_' + node_id in self._level_bonds[_l]:
                                _notched_bonds[neigh_id + '_' + node_id] = self._level_bonds[_l].pop(
                                    neigh_id + '_' + node_id)
        ...
        if self._damage:
            ...
            while _l >= 0:
                ...
                for _bond_id, _bond in self._level_bonds[_l].iteritems():
                    ...
                    if numpy.random.rand() < self._damage_fraction and _n1._id + '_' + _n2._id not in _damaged_bonds \
                            and _n2._id + '_' + _n1._id not in _damaged_bonds:
                        _n1.debond_from(_n2)
    ...
    def _solve(self):
        ...
        while _l >= 0:
            if not self._level_is_solved[_l]:
                ...
                _N_interior, _N_exterior_up, _N_exterior_dn = _eqns['_N_interior'], _eqns['_N_exterior_up'], _eqns[
                    '_N_exterior_dn']
    ...
    def _check(self, verbose=False):
        ...
        while _l >= 0:
            ...
            _N_interior, _N_exterior_up, _N_exterior_dn = _eqns['_N_interior'], _eqns['_N_exterior_up'], _eqns[
                '_N_exterior_dn']
            _interior_node_ids, _exterior_up_node_ids, _exterior_dn_node_ids = _eqns['interior_node_ids'], _eqns[
                'exterior_up_node_ids'], _eqns['exterior_dn_node_ids']
            _interior_node_indices, _exterior_up_node_indices, _exterior_dn_node_indices = _eqns[
                                                                                               'interior_node_indices'], \
                                                                                           _eqns[
                                                                                               'exterior_up_node_indices'], \
                                                                                           _eqns[
                                                                                               'exterior_dn_node_indices']
            ...
            for i in range(N):
                ...
                for neigh_id in self._level_nodes[_l][node_id]._bonded_to:
                    ...
                    if verbose:
                        neigh_x, neigh_y = [int(z) for z in neigh_id.split('_')[1:]]
                        print '\t Neighbor: (%d,%d), vol: %.2f, vol_diff: %.2f, current: %.2f' % (
                            neigh_x, neigh_y, neigh_vol, node_vol - neigh_vol, neigh_curr)

                    if abs(neigh_curr - (node_vol - neigh_vol)) > 1E-3:
                        print 'Checking node ' + node_id
                        print 'Node Voltage = %.2f' % (node_vol)
                        print '\t Neigh: %s at voltage %.2f. Vol_diff = %.2f, curr = %.2f' % (
                            neigh_id, neigh_vol, node_vol - neigh_vol, neigh_curr)
        ...
        while _l >= 0:
            if self._level_not_broken[_l]:
                ...
                if len(self._level_strain_evp[_l]) == 0:
                    ...
                    self._level_strain_evp[_l].append(
                        EY / _max_curr if EY / _max_curr > self._level_strain_evp[_l][-1] else
                        self._level_strain_evp[_l][-1])
    ...
    def bond_current(self, bond, l):
        ...
        _interior_node_ids, _exterior_up_node_ids, _exterior_dn_node_ids = _eqns['interior_node_ids'], _eqns[
            'exterior_up_node_ids'], _eqns['exterior_dn_node_ids']
        _interior_node_indices, _exterior_up_node_indices, _exterior_dn_node_indices = _eqns['interior_node_indices'], \
                                                                                       _eqns[
                                                                                           'exterior_up_node_indices'], \
                                                                                       _eqns['exterior_dn_node_indices']
        N = _N_interior + _N_exterior_dn + _N_exterior_up
        ...
        while _l >= 0:
            ...
            for i in range(N):
                ...
                for neigh_id in self._level_nodes[_l][node_id]._bonded_to:
                    ...
                    c_mat[_eqns['_j_range'] - 1 - (_ij - _eqns['_j_offset']), _ii - _eqns['_i_offset']] += abs(
                        curr[i, j])
                    c_mat[_eqns['_j_range'] - 1 - (_jj - _eqns['_j_offset']), _ji - _eqns['_i_offset']] += abs(
                        curr[j, i])
            ...
            # ax.matshow(c_mat.T)
            ax.imshow(c_mat, interpolation='nearest', cmap=cm.jet, alpha=1.0,
                      extent=[0, _eqns['_i_range'] * 0.5, 0, _eqns[
                          '_j_range'] * (3.0 ** 0.5) * 0.5])
            # ,extent=[0,cr.cell[0,0],0,cr.cell[1,1]],origin='lower',vmax=kwargs.get('vmax',120),vmin=kwargs.get(
            # 'vmin',-60))
            mpl.pyplot.draw()
    ...
    def _build_eqns(self):
        ...
        while _l >= 0:
            if not self._level_is_build[_l]:
                ...
                _Li, _Lj, _L_data = [], [], []
                _Bi, _Bj, _B_data = [], [], []
                # Sparse matrix of voltages along the diagonals. Initiated to have interior = -1, exterior up at
                # applied, exterior down at 0
                _Vi, _Vj, _V_data = [], [], []
                ...
                V = scipy.sparse.csc_matrix((_V_data, (_Vi, _Vj)), shape=(
                    _N_interior + _N_exterior_dn + _N_exterior_up, _N_interior + _N_exterior_dn + _N_exterior_up))
                ...
                C = scipy.sparse.csc_matrix((_C_data, (_Ci, _Cj)), shape=(
                    _N_interior + _N_exterior_dn + _N_exterior_up, _N_interior + _N_exterior_dn + _N_exterior_up))
    ...
    def plot(self, **kwargs):
		...
		while _l <= self._levels:
			...
			if kwargs.get('show_lines',False):
				for _b_id, _b in self._level_bonds[_l].iteritems():
					_tX,_tY = _b.coordinates(self._nx*self._l0*(self._magnification**(self._levels+1)),
					self._ny*self._l0*(self._magnification**(self._levels+1)))
					for _X, _Y in zip(_tX,_tY):
						line, = ax.plot(_X,_Y,'b-')
        ...
        while _l <= self._levels:
                    ...
                    if j_0 == j_01:
                        ...
                        if False not in [n_id in self._level_nodes[_l] for n_id in [n0_id, n1_id, n2_id]]:
                            ...
                            if n0.is_bonded_to(n1) and n0.is_bonded_to(n2) and n1.is_bonded_to(n2):
                                ...
                                patches.append(
                                    mpl.patches.Polygon(coords, closed=True, facecolor=color, edgecolor='none'))
            ...
            mpl.pyplot.draw()
            if self._notch:
                # fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.eps'%(self._nx, self._ny,_l,self._magnification,
                # self._level_notch_len[_l]),format='eps',pad_inches=0,bbox_inches='tight',dpi=144)
                fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.png' % (
                    self._nx, self._ny, _l, self._magnification, self._level_notch_len[_l]), format='png', pad_inches=0,
                            bbox_inches='tight', dpi=144)
                fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.pdf' % (self._nx, self._ny, _l, self._magnification,
                                                                          self._level_notch_len[_l]), format='pdf',
                            pad_inches=0, bbox_inches='tight', dpi=144)
            else:
                # fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.eps'%(self._nx, self._ny,_l,self._magnification,
                # 0),format='eps',pad_inches=0,bbox_inches='tight',dpi=144)
                fig.savefig(
                    'Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.png' % (self._nx, self._ny, _l, self._magnification, 0),
                    format='png', pad_inches=0, bbox_inches='tight', dpi=144)
                fig.savefig(
                    'Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f.pdf' % (self._nx, self._ny, _l, self._magnification, 0),
                    format='pdf', pad_inches=0, bbox_inches='tight', dpi=144)
        ...
        for lev_size in level_size[lev]:
            for damage in level_damage[lev]:
                ...
                for sim in range(level_sims[lev]):
                    ...
                    while not simulated:
                        try:
                            hg = hierarchical_grid(lev_size, lev_size, lev, l0=1, notch=False, damage=True,
...
def stress(hg, l):
    ...
    for _b_id, _b in hg._level_bonds[l].iteritems():
        ...
        if n1_x - _i_min >= _notch_len_ind and n2_x - _i_min >= _notch_len_ind and (n1_y - _j_mid) * (
                    n2_y - _j_mid) < 0:
...
def fit_str(mag, damage, strength, strength_den, std_strength, std_strength_den):
    c = scipy.optimize.leastsq(str_diff, numpy.array([0.12, 0.05, 2.0, 1.0]),
                               (mag, damage, strength, strength_den, std_strength, std_strength_den))[0]
    return c
...
def plot_stress(hg, l=0, x0=-1):
    ...
    fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress.png' % (
        hg._nx, hg._ny, l, hg._magnification, hg._level_notch_len[l]), format='png', pad_inches=0, bbox_inches='tight',
                dpi=144)
    fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress.pdf' % (
        hg._nx, hg._ny, l, hg._magnification, hg._level_notch_len[l]), format='pdf', pad_inches=0, bbox_inches='tight',
                dpi=144)
    ...
    [tick.label.set_fontsize(20) for tick in ax.yaxis.get_major_ticks()]
    [tick.label.set_fontsize(20) for tick in ax.xaxis.get_major_ticks()]
    fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress_Zoom.png' % (
        hg._nx, hg._ny, l, hg._magnification, hg._level_notch_len[l]), format='png', pad_inches=0, bbox_inches='tight',
                dpi=144)
    fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress_Zoom.pdf' % (
        hg._nx, hg._ny, l, hg._magnification, hg._level_notch_len[l]), format='pdf', pad_inches=0, bbox_inches='tight',
                dpi=144)
...
def toughness_stress(mag=None, toughness_fit=None, toughness_notch=None, density=None, levels=1):
	...
	if mag is None:
		...
		[tick.label.set_fontsize(20) for tick in ax.xaxis.get_major_ticks()]
		mpl.pyplot.draw()
		fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress.png'%(hg._nx, hg._ny,l,hg._magnification,
		hg._level_notch_len[l]),format='png',pad_inches=0,bbox_inches='tight',dpi=144)
		fig.savefig('Nx_%d_Ny_%d_Lev_%d_Mag_%d_Notch_%.2f_Stress.pdf'%(hg._nx, hg._ny,l,hg._magnification,
		hg._level_notch_len[l]),format='pdf',pad_inches=0,bbox_inches='tight',dpi=144)
    ...
    if 1 == 2:
        ...
        for i, df in enumerate(damage_f):
            ...
            for m, den in zip(mag, density):
                ...
                _sdf = 1.0 - _cdf
                ax.plot((1.0 / (m * m)) * ((1.0 / stress) ** 2.0) * (-7 * numpy.log(df) - numpy.log(m)), numpy.log(
                    -numpy.log(_sdf) / (m * m)), 'k.')
